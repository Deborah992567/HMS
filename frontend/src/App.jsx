...
function tokenRoles(token) {
  try { return JSON.parse(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))).roles || [] } catch { return [] }
}
...
export default function App() {
  ...
  const loadChain = async () => { setLoading(true); try { setChain(await request('/blockchain/chain')) } catch (error) { showToast(error.message, 'error') } finally { setLoading(false) } }
  ...
  const loadEhrs = async patient => { setLoading(true); try { const data = await request(`/patients/${patient.id}/timeline`); setTimeline(data); setEhrs(data.ehrs || []); setSelectedPatient(data.patient); setPage('ehrs') } catch (error) { showToast(error.message, 'error') } finally { setLoading(false) } }
  ...
  const loadAdmin = async () => { setLoading(true); try { const [doctorData, serviceData] = await Promise.all([request('/doctors/'), request('/services/'), request('/staff/')]); setAdminDoctors(doctorData); setAdminServices(serviceData); setPage('admin') } catch (error) { showToast('Admin access required: ' + error.message, 'error') } finally { setLoading(false) } }
  ...
  async function bookAppointment(event) {
    ...
    try { const booking = await readResponse(await fetch(`${apiBase}/patient/appointments/`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${patientToken}` }, body: JSON.stringify({ date: appointmentDate, doctor_id: Number(doctorId), service_id: serviceId ? Number(serviceId) : null }) })); showToast(`Appointment booked. A $${Number(booking.billing.amount).toFixed(2)} booking fee is ready to pay.`, 'success'); setAppointmentDate(''); loadBookingData() } catch (error) { showToast(error.message, 'error') }
  }
  ...
  async function createDoctor(event) {
    event.preventDefault()
    try { await request('/doctors/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newDoctorName, specialty: newDoctorSpecialty }) }); setNewDoctorName(''); setNewDoctorSpecialty(''); showToast('Doctor added.', 'success'); loadAdmin() } catch (error) { showToast(error.message, 'error') }
  }
  async function createService(event) {
    event.preventDefault()
    try { await request('/services/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newServiceName, duration_minutes: Number(newServiceDuration) || 30 }) }); setNewServiceName(''); setNewServiceDuration('30'); showToast('Service added.', 'success'); loadAdmin() } catch (error) { showToast(error.message, 'error') }
  }
  ...
  const logout = () => { localStorage.removeItem('hms_token'); localStorage.removeItem('hms_patient_token'); setToken(null); setPatientToken(null); setPage('home'); showToast('You have been signed out.') }

  return <div className="app-shell">
    <Header page={page} signedIn={Boolean(token)} patientSignedIn={Boolean(patientToken)} isAdmin={tokenRoles(token).includes('Admin')} onNav={navigate} onLogout={logout} />
    <main className="main-content">
      ...
      {page === 'book' && <section className="page-section"><div className="section-heading"><p className="eyebrow">Patient portal</p><h1>Appointments and payments</h1><p>Book care, track upcoming visits and securely pay your own bills.</p></div>{!patientToken ? <article className="form-card" style={{maxWidth: 480}}><h2>Patient sign in</h2><form onSubmit={patientLogin}><label>Email<input required type="email" value={patientEmail} onChange={e => setPatientEmail(e.target.value)} placeholder="you@example.com" /></label><label>Password<input required type="password" value={patientPassword} onChange={e => setPatientPassword(e.target.value)} placeholder="Your password" /></label><button className="button primary full">Access patient portal</button></form><p className="form-note">Need an account? <button className="text-button" onClick={() => navigate('register')}>Register as a patient</button></p></article> : <><div className="record-layout"><article className="form-card"><h2>New appointment</h2><p className="card-copy">A $50.00 booking fee will be added when you confirm.</p><form onSubmit={bookAppointment}><label>Service<select required value={serviceId} onChange={e => { setServiceId(e.target.value); setDoctorId('') }}><option value="">Select a service</option>{services.map(service => <option key={service.id} value={service.id}>{service.name} · {service.duration_minutes} min</option>)}</select></label><label>Doctor<select required value={doctorId} onChange={e => setDoctorId(e.target.value)}><option value="">Choose a doctor</option>{doctors.filter(doctor => !serviceId || doctor.specialty === services.find(service => service.id === Number(serviceId))?.name).map(doctor => <option key={doctor.id} value={doctor.id}>{doctor.name}</option>)}</select></label><label>Date and time<input required type="datetime-local" value={appointmentDate} onChange={e => setAppointmentDate(e.target.value)} /></label><button className="button primary full">Book appointment</button></form></article><div><h2>Your appointments</h2>{loading ? <EmptyState message="Loading booking options…" /> : patientAppointments.length ? patientAppointments.map(appointment => <article className="record-card" key={appointment.id}><span className="record-id">APPOINTMENT #{appointment.id}</span><h3>{new Date(appointment.date).toLocaleString()}</h3><p>Doctor #{appointment.doctor_id} · {appointment.status}</p></article>) : <EmptyState message="You have no upcoming appointments." />}</div></div><div className="section-heading" style={{marginTop: 46}}><p className="eyebrow">Your booking charges</p><h2>Pay securely (simulation)</h2><p>Payment details are encrypted before a privacy-preserving reference is recorded in the local audit chain.</p></div>{patientBills.length ? <div className="data-grid">{patientBills.map(bill => <article className="patient-card" key={bill.id}><div className="avatar">$</div><div><h3>${Number(bill.amount).toFixed(2)}</h3><p>{bill.description || `Bill #${bill.id}`}</p><span>{bill.status}</span></div>{bill.status === 'paid' ? <span className="anchor">Paid</span> : <button className="button primary" onClick={() => payPatientBill(bill.id)}>Pay now</button>}</article>)}</div> : <EmptyState message="You have no bills at the moment." />}</>}</section>}
      {page === 'admin' && <section className="page-section"><div className="section-heading"><p className="eyebrow">Administration</p><h1>Hospital directory</h1><p>Admin-only controls for managing the services patients can book and the clinicians they can choose.</p></div><div className="workflow-grid"><article className="form-card"><h2>Add a real doctor</h2><form onSubmit={createDoctor}><label>Doctor name<input required value={newDoctorName} onChange={e => setNewDoctorName(e.target.value)} placeholder="Dr. Jane Smith" /></label><label>Service / specialty<select required value={newDoctorSpecialty} onChange={e => setNewDoctorSpecialty(e.target.value)}><option value="">Select service</option>{adminServices.map(service => <option key={service.id} value={service.name}>{service.name}</option>)}</select></label><button className="button primary full">Add doctor</button></form></article><article className="form-card"><h2>Add a service</h2><form onSubmit={createService}><label>Service name<input required value={newServiceName} onChange={e => setNewServiceName(e.target.value)} placeholder="e.g. Dialysis" /></label><label>Appointment duration<input required inputMode="numeric" value={newServiceDuration} onChange={e => setNewServiceDuration(e.target.value)} /></label><button className="button primary full">Add service</button></form></article></div><div className="record-layout" style={{marginTop: 22}}><article className="form-card"><h2>Services ({adminServices.length})</h2>{adminServices.map(service => <div className="record-card" key={service.id}><h3>{service.name}</h3><p>{service.duration_minutes} minutes</p><button className="text-button" onClick={() => editService(service)}>Edit</button> · <button className="text-button" onClick={() => deleteService(service.id)}>Delete</button></div>)}</article><article className="form-card"><h2>Doctors ({adminDoctors.length})</h2>{adminDoctors.map(doctor => <div className="record-card" key={doctor.id}><h3>{doctor.name}</h3><p>{doctor.specialty || 'General'}</p><button className="text-button" onClick={() => editDoctor(doctor)}>Edit</button> · <button className="text-button" onClick={() => deleteDoctor(doctor.id)}>Delete</button></div>)}</article></div></section>}
      ...
      {page === 'ehrs' && <section className="page-section"><div className="section-heading row"><div><p className="eyebrow">Patient workspace</p><h1>{selectedPatient?.name || 'Patient'}’s timeline</h1><p>Patient #{selectedPatient?.id} · a unified view of clinical care and administration.</p></div><button className="button secondary" onClick={() => selectedPatient && loadEhrs(selectedPatient)}>Refresh</button></div><div className="timeline-summary"><article><span>Clinical records</span><strong>{ehrs.length}</strong></article><article><span>Appointments</span><strong>{timeline?.appointments?.length || 0}</strong></article><article><span>Prescriptions</span><strong>{timeline?.prescriptions?.length || 0}</strong></article><article><span>Outstanding bills</span><strong>{timeline?.billing?.filter(bill => bill.status !== 'paid').length || 0}</strong></article></div><div className="patient-timeline"><div className="timeline-column"><h2>Clinical records</h2>{ehrs.length ? ehrs.map(record => <article className="record-card" key={record.id}><span className="record-id">RECORD #{record.id}</span><h3>{record.diagnosis}</h3><p><strong>Medication:</strong> {record.medication || 'Not recorded'}</p>{record.notes && <p className="notes">{record.notes}</p>}</article>) : <EmptyState message="No clinical records have been created for this patient." />}</div><aside><article className="form-card"><h2>New clinical record</h2><EHRForm patientId={selectedPatient?.id} onCreate={payload => submit('/ehr/', payload, 'Clinical record created.', () => selectedPatient && loadEhrs(selectedPatient))} /></article><div className="timeline-side-card"><h2>Appointments</h2>{timeline?.appointments?.length ? timeline.appointments.map(item => <p key={item.id}><strong>{new Date(item.date).toLocaleDateString()}</strong><br />Doctor #{item.doctor_id} · {item.status}</p>) : <p>No appointments recorded.</p>}</div><div className="timeline-side-card"><h2>Prescriptions</h2>{timeline?.prescriptions?.length ? timeline.prescriptions.map(item => <p key={item.id}><strong>{item.medication}</strong><br />{item.dosage}{item.fulfilled ? ' · Fulfilled' : ''}</p>) : <p>No prescriptions recorded.</p>}</div><div className="timeline-side-card"><h2>Billing</h2>{timeline?.billing?.length ? timeline.billing.map(item => <p key={item.id}><strong>${Number(item.amount).toFixed(2)}</strong><br />Bill #{item.id} · {item.status}</p>) : <p>No billing records.</p>}</div></aside></div></section>}
      ...
      {page === 'assistant' && <section className="page-section"><div className="section-heading"><p className="eyebrow">Assistant</p><h1>Your care operations copilot.</h1><p>Ask about Careflow workflows or general health information. It runs privately through your local Ollama instance.</p></div><Chat apiBase={apiBase} /></section>}
